...
pub async fn run(
    ...
    let iq: f64 = 
        if CONFIG.rig_ids.contains(&user.id.get()) {
            Normal::new(100000000.0, 10000000.0).unwrap()
                .sample(&mut rand::thread_rng())
        } else {
            Normal::new(100.0, 25.0).unwrap()
                .sample(&mut rand::thread_rng())
        };
    ...
    let emoji = if iq >= 110.0 { &CONFIG.emojis.bigbrain } else if 90.0 >= iq { &CONFIG.emojis.nobrain } else { &CONFIG.emojis.justbrain };
